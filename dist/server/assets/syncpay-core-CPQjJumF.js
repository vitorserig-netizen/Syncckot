...
const checkoutSchema = objectType({
  name: stringType().min(3, "Informe seu nome completo").max(120),
  email: stringType().email("Informe um email válido").max(160),
  cpf: stringType().regex(/^\d{11}$/, "CPF deve conter 11 números"),
  phone: stringType().regex(/^\d{10,13}$/, "Celular deve conter DDD e número"),
  includeAddon: booleanType().default(false),
  phoneModel: stringType(),
  browser: stringType(),
  batteryLevel: numberType().min(0).max(100)
});
function getClientIp(request) {
  return request?.headers.get("cf-connecting-ip") ?? request?.headers.get("x-forwarded-for")?.split(",")[0]?.trim() ?? "127.0.0.1";
}
async function readSyncPayError(response) {
...
async function createPixPaymentForCheckout(input, request) {
  ...
  const trustedDescription = data.includeAddon ? "Odair José - A Promessa Que Fiz Por Amor (Livro Digital) + apoio ao livro físico" : "Odair José - A Promessa Que Fiz Por Amor (Livro Digital)";
  ...
  const origin = request ? new URL(request.url).origin : void 0;
  let cashInResponse = await fetch(`${syncPayApiBase}/api/partner/v1/cash-in`, {
    ...
    body: JSON.stringify({
      ...
      webhook_url: origin ? `${origin}/api/public/syncpay-webhook` : void 0,
      client: {
        name: data.name,
        cpf: data.cpf,
        email: data.email,
...
export {
  createPixPaymentForCheckout as a,
  checkoutSchema as c
};
